	...
	$(document).ready(function() {
		$("#input_week").on("click", function() {
		    ...
		    $("#weekly").fullCalendar({
		        ...
		        dayClick: function(date, jsEvent, view) {
		        	...
		        	$("#input_field").html("Worked Hours: <input id='input_tag' name='worked_hour' type='number' min='0' max=''></div>");
		        	...
		        	$("#input_tag").on("blur", function() {
		            	...
		            	$("td[data-date='"+detail_date+"']").append("Work Hour: <span id='hour_in_"+detail_date+"'>"+hours+"</span>");
		          	});
		        }
		    });
		    $(".fc-day").each(function() {
		        ...
		        $.ajax({
	      			...
	      			success: function(response) {
	      				var isHoliday = response.toString().trim();
	      				if(isHoliday!="notHoliday") {
	      					$("td[data-date='"+date+"']").append("<div>Holiday "+isHoliday+"</div><br/>");
	      				}
	      			}
		        });
		    });
		...
		$("#input_month").on("click", function() {
		    ...
		    $("#monthly").fullCalendar({
		        ...
		        dayClick: function(date, jsEvent, view) {
		        	$(".fc-day").removeClass("selected");
		        	$("#input_mon_field").html("Worked Hours: <input id='input_mon_tag' name='worked_hour' type='number' min='0' max='24'></div>");
		            ...
		            $.ajax({
		      			...
		      			success: function(response) {
		      				var isHoliday = response.toString().trim();
		      				if(isHoliday!="notHoliday") {
		      					$("#input_mon_field").prepend("<div>This day is a holiday "+isHoliday+"</div>");
		      				}
		      			}
		      	    });
		            $.ajax({
		          ...
		          // change the day's background color just for fun
		            ...
		            $("#input_mon_tag").on("blur", function() {
		            	...
		            	$("td.fc-day[data-date='"+detail_date+"']").html("<br/>Work Hour: <div id='hour_in_mon_"+detail_date+"'>"+hours+"</div>");
		            });
		        }
		    });
		});
